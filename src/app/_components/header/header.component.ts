...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  constructor(private userService: UserService, public overlaysService: OverlaysService, private phraseConfirm: PhraseConfirmService, private midgardService: MidgardService, private _decimalPipe: DecimalPipe) {
  ...
  getPoolCap() {
    ...
    const sub = combined.subscribe( ([mimir, network]) => {
      ...
      if (mimir && mimir['mimir//MAXIMUMLIQUIDITYRUNE']) {
        ...
        this.topbar = `${this._decimalPipe.transform(this.totalPooledRune, '0.0-0')} / ${this._decimalPipe.transform(this.maxLiquidityRune, '0.0-0')} RUNE POOLED`
      }
